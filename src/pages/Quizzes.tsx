  ...
  const LoginGate = ({ title }: { title: string }) => (
    <div className="bg-white/70 backdrop-blur-xl p-12 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white text-center">
      <div className="max-w-md mx-auto">
        ...
        <h2 className="text-3xl font-extrabold text-blue-950 mb-3">{title}</h2>
      ...
      {/* Navigation Tabs */}
        <div className="flex overflow-x-auto whitespace-nowrap border-b border-white/60">
          ...
          <button
            ...
            className={`py-3 px-6 font-bold text-sm border-b-2 transition-colors ${activeTab === 'marked' ? 'border-blue-600 text-blue-700' : 'border-transparent text-blue-900/50 hover:text-blue-900/80'}`}
          >
        ...
        <div className="space-y-4">
          {!user ? (
            ...
            <div className="space-y-6">
              <div className="bg-white/60 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white space-y-4">
                <div>
                  <h2 className="text-2xl font-extrabold text-blue-950">{t('marked_words') || 'Marked words'}</h2>
                  <p className="text-lg text-blue-900/70 font-medium">{t('marked_words_description') || t('marked_words_subtitle')}</p>
                </div>
                ...
                {userVocabulary === null ? (
                  ...
                  <div className="text-center py-10 px-4">
                    ...
                    <h3 className="text-xl font-extrabold text-blue-950 mb-2">{t('marked_words_empty') || 'No marked words yet'}</h3>
                    <p className="text-blue-900/70 font-medium max-w-lg mx-auto">
                    ...
                    {Array.from({ length: markedLevels }, (_, i) => i + 1).map((quizId) => {
                      ...
                      return (
                        <Link
                          ...
                          className="group flex items-center justify-between p-5 rounded-2xl bg-white/90 border border-blue-50 hover:border-blue-200 hover:shadow-md transition-all"
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-11 h-11 rounded-xl bg-amber-50 text-amber-500 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">⭐</div>
                            <div>
                              <h3 className="font-extrabold text-blue-950 group-hover:text-blue-700 transition-colors">
              ...
              {/* Manage library — remove one by one */}
              <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-6 md:px-8 hover:bg-white/40 transition-colors outline-none"
                >
                  <div className="text-left">
                    <h3 className="text-xl font-extrabold text-blue-950">{t('marked_words_manage') || 'Manage library'}</h3>
                    <p className="text-sm text-blue-900/60 font-medium mt-0.5">
                      {t('marked_words_manage_desc') || 'Remove words one by one from your marked list only. Public library quizzes are never changed.'}
                    </p>
                  </div>
                  <div className={`w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm text-blue-600 transition-transform duration-500 ${manageOpen ? 'rotate-180' : ''}`}>
                ...
                {manageOpen && (
                  <div className="border-t border-blue-50/50">
                    {markedWords.length === 0 ? (
                      <p className="p-8 text-center text-blue-900/60 font-medium">{t('marked_words_empty')}</p>
                    ) : (
                              ...
                              <tr key={word.id} className="hover:bg-white/60 transition-colors">
                                ...
                                <td className="p-2 sm:p-4 text-center">
                                  <button
                                    ...
                                    className="text-red-500 hover:text-red-700 p-1.5 sm:p-2 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-40"
