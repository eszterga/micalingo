...
export default function Import() {
  ...
  const handleDownloadTemplate = (type: 'standard' | 'articles' | 'false_friends') => {
    ...
    if (type === 'articles') {
      ...
      templateData = [
    { [t('template_german_header')]: "das Gift, die Gifte", [t('template_hungarian_header')]: "a méreg", [t('template_example_header')]: "Dieses Tier produziert ein starkes Gift.", [t('template_note_header')]: "False friend: 'Gift' means poison in German!" },
      ];
    } else {
      templateData = [
                ...
                <tbody className="divide-y divide-gray-100">
                  {previewItems.map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                      {destination === 'articles' ? (
                        <>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={(item as any).article || ''} onChange={(e) => handleItemChange(idx, 'article', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={(item as any).noun || ''} onChange={(e) => handleItemChange(idx, 'noun', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.hungarian || ''} onChange={(e) => handleItemChange(idx, 'hungarian', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 align-top"><textarea rows={2} value={item.example || ''} onChange={(e) => handleItemChange(idx, 'example', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                        </>
                      ) : destination === 'false_friends' ? (
                        <>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.german || ''} onChange={(e) => handleItemChange(idx, 'german', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.hungarian || ''} onChange={(e) => handleItemChange(idx, 'hungarian', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.example || ''} onChange={(e) => handleItemChange(idx, 'example', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          <td className="p-1 sm:p-2 align-top"><textarea rows={2} value={(item as any).note || ''} onChange={(e) => handleItemChange(idx, 'note', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                        </>
                      ) : (
                        <>
                          <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.german || ''} onChange={(e) => handleItemChange(idx, 'german', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
            ...
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-2">
                <button
                  ...
                  disabled={saving}
                   className="flex items-center justify-center gap-2 bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-lg font-bold transition-colors shadow-sm disabled:opacity-50"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  {t('download_selected')}
                </button>
                ...
                {filteredImportedFiles.map((file) => (
                  <tr key={file.fileName} className="hover:bg-gray-50 transition-colors">
                    <td className="p-2 sm:p-4 text-center">
                      <input type="checkbox" checked={selectedFiles.has(file.fileName)} onChange={() => toggleFileSelection(file.fileName)} className="w-5 h-5 text-blue-600 rounded border-gray-300 cursor-pointer" />
                    </td>
                  ...
                  <td className="p-2 sm:p-4 text-right">
                    <div className="flex items-center justify-end gap-1 sm:gap-2">
                       <button onClick={() => handleEditFile(file)} disabled={saving} className="flex items-center text-green-600 hover:text-green-800 p-1.5 sm:p-2 rounded hover:bg-green-50 transition-colors disabled:opacity-50">
                        <svg className="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        <span className="hidden sm:inline">Edit</span>
                      </button>
                       <button onClick={() => handleDownloadFiles([file.fileName])} disabled={saving} className="flex items-center text-blue-600 hover:text-blue-800 p-1.5 sm:p-2 rounded hover:bg-blue-50 transition-colors disabled:opacity-50">
                       ...
                       <button onClick={() => handleDeleteFile(file.fileName, file.wordIds)} disabled={saving} className="flex items-center text-red-600 hover:text-red-800 p-1.5 sm:p-2 rounded hover:bg-red-50 transition-colors disabled:opacity-50">
                        <svg className="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 transition-opacity">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {editFileItems.map((item, idx) => (
                      <tr key={item.id || idx} className="hover:bg-gray-50 transition-colors">
                        {editingFileCategory === 'articles' ? (
                          <>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={(item as any).article || ''} onChange={(e) => handleEditItemChange(idx, 'article', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={(item as any).noun || ''} onChange={(e) => handleEditItemChange(idx, 'noun', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.hungarian || ''} onChange={(e) => handleEditItemChange(idx, 'hungarian', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 align-top"><textarea rows={2} value={item.example || ''} onChange={(e) => handleEditItemChange(idx, 'example', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          </>
                        ) : editingFileCategory === 'false_friends' ? (
                          <>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.german || ''} onChange={(e) => handleEditItemChange(idx, 'german', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.hungarian || ''} onChange={(e) => handleEditItemChange(idx, 'hungarian', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.example || ''} onChange={(e) => handleEditItemChange(idx, 'example', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                            <td className="p-1 sm:p-2 align-top"><textarea rows={2} value={(item as any).note || ''} onChange={(e) => handleEditItemChange(idx, 'note', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
                          </>
                        ) : (
                          <>
                            <td className="p-1 sm:p-2 border-r border-gray-100 align-top"><textarea rows={2} value={item.german || ''} onChange={(e) => handleEditItemChange(idx, 'german', e.target.value)} className="w-full border border-gray-200 rounded p-1 text-sm resize-none" /></td>
            ...
            <div className="p-4 border-t border-gray-200 flex justify-end gap-3 bg-gray-50 rounded-b-xl">
              ...
              <button onClick={handleSaveFileEdits} disabled={isSavingEdit} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-sm disabled:opacity-50 flex items-center justify-center gap-2">
