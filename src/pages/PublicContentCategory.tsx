                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
                    ...
                    {((user && item.userId === user.uid) || (isAdmin && adminMode && item.userId === "PUBLIC_LIBRARY")) && (
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end sm:border-l sm:border-gray-100 sm:pl-4 pt-3 sm:pt-0 border-t border-gray-100 sm:border-t-0 flex-shrink-0">
                        <button onClick={() => openEditModal(item)} className="px-3 py-2 sm:p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 hover:text-blue-700 transition-colors shadow-sm flex items-center gap-2" title={t((type === 'articles' ? "edit_article" : type === 'books' ? "edit_book" : "edit_interesting") as any)}>
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                          <span className="text-xs font-bold sm:hidden">{t("edit_word") || "Edit"}</span>
                        </button>
                        <button onClick={() => handleDelete(item.id)} className="px-3 py-2 sm:p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 hover:text-red-600 transition-colors shadow-sm flex items-center gap-2" title={t((type === 'articles' ? "delete_article" : type === 'books' ? "delete_book" : "delete_interesting") as any)}>
