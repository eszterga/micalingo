                        ...
                        {cat.items.map((item: any) => {
                          ...
                          return (
                            <div key={item.id} className="relative bg-white/90 backdrop-blur-xl p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-blue-50 transition-all duration-300 group/item">
                              ...
                              <button onClick={() => setExpandedItems(prev => { const n = new Set(prev); if(n.has(item.id)) n.delete(item.id); else n.add(item.id); return n; })} className="w-full flex items-center justify-between group outline-none text-left pr-20 md:pr-[90px]">
